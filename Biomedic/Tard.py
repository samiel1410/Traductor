import requests, json, xmltodict, numpy
...
def all(alineamiento, resul, output, cadena):
    ...
    return resultantList_pala_com,arry2,array_espanol,codigo_unidos,coincidencia_unidos,candidate_score_array,posicion_final

def imprimir(alineamiento, resul, output, cadena):
    resultantList_pala_com, arry2, array_espanol, codigo_unidos, coincidencia_unidos, candidate_score_array,posicion_final=all(alineamiento, resul, output, cadena)
    entity_var=[]
    cui=[]
    for i in range(len(resultantList_pala_com)):
        #print("Palabra #", i + 1)
